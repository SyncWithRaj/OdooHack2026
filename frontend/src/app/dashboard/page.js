  ...
  if (authLoading || !user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface">
        ...
        >
          <div className="w-12 h-12 rounded-lg bg-accent/15 border border-accent/30 flex items-center justify-center shadow-sm">
            <div className="w-5 h-5 bg-accent rounded-sm animate-ping" />
          </div>
          <span className="text-xs font-bold uppercase tracking-widest text-steel mt-2">Loading AssetFlow...</span>
        </motion.div>
      </div>
    );
  }
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div 
                    ...
                    className="absolute right-0 mt-2 w-80 bg-white border border-hairline rounded-lg shadow-xl py-2 z-50"
                  >
